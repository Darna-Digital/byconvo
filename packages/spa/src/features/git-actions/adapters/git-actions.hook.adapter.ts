...
const unwrap = async <T>(p: Promise<{ data?: T; error?: unknown }>): Promise<T> => {
  ...
  if (error) throw new Error((error as { message?: string; reason?: string }).message ?? (error as { reason?: string }).reason ?? "request failed")
  return data as T
}
...
export function useGitActions() {
      ...
      createGitActionsFunctions({
        ...
        sideEffects: {
          commit: (message, paths) =>
            unwrap(fetchClient.POST("/api/commit", { body: { message, paths: [...paths] } })),
  ...
  const post = <T>(p: Promise<{ data?: T; error?: unknown }>) => () => unwrap(p)
  ...
  return {
    ...
    checkoutAndUpdate: async (branch: string) => {
      try {
        ...
        notify("ok", output.length > 0 ? output : `Checked out and updated ${branch}`)
        refresh()
      } catch (cause) {
    ...
    createBranch: (name: string, startPoint: string | null) =>
      fns.runOp(`Created branch ${name}`, () =>
        unwrap(fetchClient.POST("/api/branch", { body: { name, startPoint: startPoint ?? undefined } })),
      ),
    ...
    merge: (branch: string) =>
      fns.runOp(`Merged ${branch}`, post(fetchClient.POST("/api/merge", { body: { branch } }))),

    rebase: (onto: string) =>
      fns.runOp(`Rebased onto ${onto}`, post(fetchClient.POST("/api/rebase", { body: { onto } }))),
