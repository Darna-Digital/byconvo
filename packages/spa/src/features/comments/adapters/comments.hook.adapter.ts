...
export function useCommentsActions() {
      ...
      createCommentsFunctions({
        ...
        sideEffects: {
          addLocalComment: async (input) => {
            const { data, error } = await fetchClient.POST("/api/comments", { body: input })
            if (error) throw new Error((error as { reason?: string }).reason ?? "failed to comment")
            return data as ReviewComment
          },
          addPullComment: async (pullNumber, input) => {
            const { data, error } = await fetchClient.POST("/api/github/pulls/{number}/comments", {
          ...
          deleteComment: async (id) => {
            await fetchClient.DELETE("/api/comments/{id}", { params: { path: { id } } })
          },
          replyPullComment: async (pullNumber, commentId, body) => {
            const { data, error } = await fetchClient.POST(
              "/api/github/pulls/{number}/comments/{commentId}/replies",
              {
                params: { path: { number: String(pullNumber), commentId: String(commentId) } },
                body: { body },
              },
            )
            if (error) throw new Error((error as { reason?: string }).reason ?? "failed to reply")
            return data as ReviewComment
          },
        },
      }),
  ...
  const invalidate = (key: string) => queryClient.invalidateQueries({ queryKey: ["get", key] })

  return {
    submit: async (ctx: SubmitContext, location: DraftLocation, body: string) => {
      const created = await fns.submit(ctx, location, body)
      void invalidate(ctx.mode === "review" ? "/api/github/pulls/{number}/comments" : "/api/comments")
      return created
    },
