...
export const GitHubController = HttpApiBuilder.group(Api, "github", (handlers) =>
  handlers
    .handle("pulls", () => Effect.flatMap(GitHubService, (s) => s.pulls))
    .handle("pullDiff", ({ params }) =>
      pullNumber(params.number).pipe(
        Effect.flatMap((n) => Effect.flatMap(GitHubService, (s) => s.pullDiff(n))),
      ),
    )
    .handle("pullComments", ({ params }) =>
      pullNumber(params.number).pipe(
        Effect.flatMap((n) => Effect.flatMap(GitHubService, (s) => s.pullComments(n))),
      ),
    )
        ...
        Effect.flatMap((n) =>
          pullNumber(params.commentId).pipe(
            Effect.flatMap((commentId) =>
              Effect.flatMap(GitHubService, (s) =>
                s.replyToPullComment({ pullNumber: n, commentId, body: payload.body }),
              ),
            ),
          ),
        ),
