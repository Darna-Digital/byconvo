...
export interface RepoRepo {
  ...
  readonly rangeDiff: (base: string, head: string) => Effect.Effect<string, GitFailure>
  ...
  readonly renameBranch: (from: string, to: string) => Effect.Effect<void, GitFailure>
  readonly deleteBranch: (name: string, force: boolean) => Effect.Effect<void, GitFailure>
}
